...
def test_sigmoid(a: float) -> None:
    ...
    * It is  strictly increasing.
    """
    sigmoid_a = sigmoid(a)
    sigmoid_neg_a = sigmoid(-1.0 * a)
    assert sigmoid_a >= 0.0 and sigmoid_a <= 1.0
    assert is_close(1.0 - sigmoid_a, sigmoid_neg_a) == 1.0
    assert sigmoid(0) == 0.5
    assert sigmoid(a + 1.0) >= sigmoid_a
...
def test_distribute(a: float, b: float, c: float) -> None:
    ...
    """
    assert is_close(mul(a, add(b, c)), add(mul(a, b), mul(a, c))) == 1.0
...
def test_sum_distribute(ls1: List[float], ls2: List[float]) -> None:
    ...
    """
    assert is_close(sum(addLists(ls1, ls2)), sum(ls1) + sum(ls2)) == 1.0
